...
//     比一阶 EMA 多一个 **vel 动量状态**——这是「完美弧线」需要、而一阶扔掉了的东西。
//   【动量弧 tail】每 push 从 (pos, vel) 的拷贝**非破坏地继续跑 SmoothDamp 飞向光标** → 一段弧（贴笔尖预览）。
//     弧来自 vel 的切向动量（顺动量冲出再弯回 = 自然弧）；直行 vel 朝光标 → 退化直线。vel 是平滑积分的连续
//     状态 → 弧帧间稳、**不闪**（v250 的 Bézier 靠几何回看重建 heading，d 随速度跳 → 乱闪，已弃）。
//     抬笔 finish() = 把这段弧整段转正 → 预览所见即所得（≈ Procreate 的动作，v244-246「完美」那版的方案）。
//
...
const FLUSH_DT = 6;       // tail flush 每 tick 的名义 dt(ms)（只决定弧采样密度，不决定弧形）
