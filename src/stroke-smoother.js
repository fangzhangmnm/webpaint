...
export class StrokeSmoother {
  ...
  _computeC(i) {
    ...
    const si = this.rs[i];
    let sumW = 0, ax = 0, ay = 0;
    // 左展开
    for (let j = i; j >= 0; j--) {
      const d = si - this.rs[j];
      if (d > W) break;
      const w = 1 - d / W; sumW += w; ax += w * this.rx[j]; ay += w * this.ry[j];
    }
    // 右展开
    for (let j = i + 1, n = this.rx.length; j < n; j++) {
      const d = this.rs[j] - si;
      if (d > W) break;
      const w = 1 - d / W; sumW += w; ax += w * this.rx[j]; ay += w * this.ry[j];
    }
    const avgx = ax / sumW, avgy = ay / sumW;
    const leftReach  = si;                  // = s_i − s_0（s_0 = 0）
    const rightReach = this.tipS - si;
    const r = Math.max(0, Math.min(1, Math.min(leftReach, rightReach) / W));
    this.cx[i] = xi + r * (avgx - xi);
    this.cy[i] = yi + r * (avgy - yi);
    this.cp[i] = this.rp[i];
  }
