...
// 笔架项的最小结构（brush 本体仍是未类型化 .js；这里只读 folder 字段）。
interface RackBrush { folder?: string; }

// 某工具的 brush 列表 → folder 名集合（保序去重；空则给默认夹）。
export function collectFolders(brushes: RackBrush[], defaultFolder: string): string[] {
...
export function brushesInFolder<T extends RackBrush>(brushes: T[], folder: string, defaultFolder: string): T[] {
...
export function smoothstepRadialGradient(hardness: number, stops = 16): string {
