...
export function mergeFolders(a, b, { resolve } = {}) {
...
export function isValidFolderEnvelope(o) {
  return !!o && typeof o === "object"
    && Number.isFinite(o.version)
    && Array.isArray(o.items) && o.items.every((e) => e && e.id != null && Number.isFinite(e.uat))
    && Array.isArray(o.trash) && o.trash.every((t) => t && t.id != null && Number.isFinite(t.uat))
    && Number.isFinite(o.resetAt);
}

// 解析不可信 text/bytes（captive-portal 的 HTML 登录页 / 慢网截断）→ 合法 envelope 或 null。
// 绝不让脏字节进 merge：调用端只在非 null 时才 merge。
export function parseFolderBlob(textOrBytes) {
  let o;
  try {
