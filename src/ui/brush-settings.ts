...
export const BrushSettings = defineComponent({
  ...
  <div>
    ...
    <div class="${SECTION}">
      ...
      <div class="${ROW}"><label>pull-stab</label><input type="range" min="0" max="1" step="0.05" v-model.number="draft.smooth.pullStabilizer"><span class="${VAL}">{{ draft.smooth.pullStabilizer.toFixed(2) }}</span></div>
      <div class="${ROW}"><label>motion-filter</label><input type="range" min="0" max="1" step="0.05" v-model.number="draft.smooth.motionFilter"><span class="${VAL}">{{ draft.smooth.motionFilter.toFixed(2) }}</span></div>
