...
// adopt 期 opts（绕开签名兼容用全局传）。
interface AdoptOpts { skipCheckpoint?: boolean; }
// .ora 反序列化出的编辑器状态（外部数据：已知字段类型化，其余宽松）。
...
// 注：store.flow.* 的结果**不**在此另立类型——_store 经 app-store.js 携带 store/** 的真类型，
//   结果变量直接 infer 真 FlowResult（自定义会与 store 的同名类型撞车）。
...
let setGalleryOpen: AppContext["setGalleryOpen"], gateCloudSyncOnOpen: AppContext["gateCloudSyncOnOpen"];
let checkQuotaAndWarn: AppContext["checkQuotaAndWarn"], uniqueLocalName: AppContext["uniqueLocalName"];
let gallery: AppContext["gallery"];   // 晚绑（gallery 后建；通过 ctx.gallery 在 init 后回填）
let getLocalSavedAtLabel: AppContext["getLocalSavedAtLabel"];   // app 的标签格式化（读 _docLastSavedAt → 经 getter）
...
let _activeSessionName: string | null = "未命名";
...
let _adoptLoadedOpts: AdoptOpts = {};
...
async function _writeSessionCheckpoint(name: string) {
  ...
  await setMeta(`revert:${name}:ora`, new Blob([sealed as BlobPart], { type: "application/zip" }));
...
function adoptLoadedDoc(loaded: LoadedDoc, sessionName: string) {
  _loadingDoc = true;
  try {
...
function adoptLoadedDocWithOpts(loaded: LoadedDoc, name: string, opts: AdoptOpts) {
...
async function renameCurrentSession({ suggested, reason }: { suggested?: string; reason?: string } = {}) {
  ...
  const oldName = _activeSessionName!;   // rename 仅在有活动画时触发（! = 类型断言，零运行时改动，保原语义）
