...
**版本** `1.3.0` · **Wire** `v1`（所有 endpoint 以 `/v1` 开头）
...
```
你的编辑器  →  BTP 客户端库  →  (HTTP / HTTPS)  →  Blender 插件  →  Blender 里的贴图
```

连接就一种心智模型——**给 `BTPClient` 一个 `baseUrl`，调用代码完全一样**：同机用 localhost（`http://127.0.0.1:18765`，开箱即用）；另一台设备就指向任何能连到该 server 的 HTTPS 地址（最省事是 `tailscale serve`，自动给机器的 `*.ts.net` 名一张有效证书、走隧道、同 WiFi 直连）。细节、安全模型、为什么跨设备得走 HTTPS——见文末「连接方式与原理」。
...
只有一件事：给 `BTPClient` 一个 `baseUrl`。其余调用代码两种连接完全一致。

```javascript
// 同机：留空走默认 http://127.0.0.1:18765
const client = new BTPClient();

// 另一台设备：指向任何能连到 Blender server 的 HTTPS 地址。
// 最省事是在 PC 上 `tailscale serve --bg https / http://127.0.0.1:18765`，
// 得到 https://<机器名>.<tailnet>.ts.net（有效证书、走隧道、同 WiFi 直连）。
const client = new BTPClient({ baseUrl: "https://pc.tailnet.ts.net" });
```

为什么另一台设备得走 HTTPS（而不能直接 `http://<局域网IP>`）——见「连接方式与原理」。
...
参考图 = 只存放置信息的元数据对象，靠名字 link 一张贴图（像素先用上面的 texture 端点发过去）。`putReference` 幂等 upsert：不存在就建、存在就重连图 + 重摆位。Blender 里实现成一个 Image Empty（打 `btp_ref=<名字>` 标记），长边归一化到 1（aspect 跟图走）。

```
listReferences()            ──  GET    /v1/references           → Reference[]
getReference(name)          ──  GET    /v1/references/{name}    → Reference
putReference(name, spec)    ──  PUT    /v1/references/{name}     幂等 upsert
  spec = { image:"<贴图名>", placement?, opacity? }
  placement = { mode:"view", view:"front|back|left|right|top|bottom" }
            | { mode:"transform", location?, rotation?, scale? }
  → Reference                  错误：404 texture_not_found（先发贴图再建参考）
deleteReference(name)       ──  DELETE /v1/references/{name}     → { deleted }
```
...
**跨设备为何走 HTTPS**：浏览器禁止 HTTPS 页面 `fetch http://<局域网IP>`（mixed-content 限制），同 WiFi 也拦，只有 `127.0.0.1` 在白名单。所以另一台设备必须用 **HTTPS** 连 server——可一旦要 HTTPS 又卡在「拿不到局域网地址的可信 TLS 证书」（公网 CA 不给私有 IP 签证书；自签则要在每台设备手动装信任 profile）。

**解法：给 server 一个能被信任的 HTTPS 地址**。最省事是 [Tailscale](https://tailscale.com)：`tailscale serve` 把本机 server 暴露在机器的 `*.ts.net` 名上，自动配 Let's Encrypt 有效证书、走 WireGuard 隧道（同 WiFi 直连、异地也通）。iPad 上不用装证书、也不撞 iOS 局域网权限（流量走隧道、不是物理局域网）。两台设备在同一 tailnet 即可。安全上 `serve`（非 `funnel`）只对自己的 tailnet 可见。

**启用 Tailscale（一次性设置）**
> as-of bundle v1.3.0 / 2026-06-27

1. PC 与 iPad 登进**同一个 tailnet**（同一 Tailscale 账号）。
2. 管理台 [admin → DNS](https://login.tailscale.com/admin/dns)：打开 **MagicDNS** 和 **HTTPS Certificates**（`serve` 配证书要这个）。
3. PC 上跑 `tailscale serve --bg 18765` —— 把本机 `127.0.0.1:18765` 暴露成 `https://<机器名>.<tailnet>.ts.net`。`--bg` 持久化跨重启；`tailscale serve status` 查地址，`tailscale serve --bg off` 撤销。（Windows CLI 在 `C:\Program Files\Tailscale\tailscale.exe`，权限报错就用管理员终端。）
4. 每开一次 Blender：N-panel 里开 "On this PC"（每会话授权一次）。插件本体无需改。
5. iPad 编辑器把那个 `*.ts.net` 地址填进「远程地址」→ 连接。同 WiFi 直连、异地走隧道。

> 早期版本用 WebRTC DataChannel 来打这堵墙，但它在 iOS 主屏 PWA 里不工作（拿不到 host ICE candidate、局域网权限不弹），已移除；旧实现存于 git tag `archive/webrtc-transport`。

**不依赖 Tailscale 的同 WiFi 直连**也可以，但得自己给 server 弄一张 iPad 信任的证书（自签 + 在 iPad 装信任 profile，或自有域名 + 真证书），更折腾——见上面的证书难点。
