...
> bufBbox 已终值）。PixelEdit 恢复纯 undo。详见 `docs/20260601-brush-frozen-tail-smoothing.md` §8.5。
...
### 油画 / 水彩 / 厚涂笔刷
- 见 docs/20260528-lessons-magic-wand-gap-closing.md 类似思路（暂不实装但留 hook）
- 厚涂需要 normal map / 法线方向
- 水彩需要 wet-edge + 干燥状态机
- v82 加了 smudge engine 后已经可以做基础混色，更细的留 v100+

### WebGL / WebGPU 加速通路
- 详 [docs/20260528-engineering-roadmap.md] 已有完整论证
...
### 无缝贴图模式（offset / wrap）
- 详 docs/20260527-artist-priorities.md Tier 2 #14
...
- 详 docs/20260527-artist-priorities.md Tier 2 #12
...
- 详 docs/20260527-artist-priorities.md Tier 3 #18
...
- 详 docs/20260527-artist-priorities.md Tier 2 #9
- 笔画末尾停住 → 自动拉直 / 拉成圆弧
...
- 详 docs/20260527-artist-priorities.md Tier 3 #16
...
- 关联 docs/20260528-engineering-roadmap.md WebGL 通路
...
- 后续真做：参考 docs/20260529-brush-architecture.md 的 smudge 通路；engine 已有 `_sampleLayerColor`，需要 mode="smudge" 的 stamp-with-loaded-color + 步长内 color decay

### 7 个 toolbar icon webui 迭代后替换（v123 记）
- 求助信 docs/20260530-icon-iteration-prompt.md：smudge / 魔棒 / Venn 并 / Venn 差 / 橡皮 / eyedropper / 图库 / 笔刷 / 套索 / 全选 共 10 个
...
- WebPaint v122 走通后，按 docs/20260529-dev-prod-split.md checklist 逐个考量 sibling：
...
### Lineart 重新着色
- 详 docs/20260527-artist-priorities.md Tier 2 #8b
...
- v71 实装 → v79 撤掉
- 详 [docs/20260528-lessons-magic-wand-gap-closing.md]
- 重做思路：EDT 距离场 + Dijkstra-like cost-flood / dead-end 桥接 / 双层混合
- 距 P2 也远
- **2026-07-30 user：stale——「线稿闭合已经 good enough」**（v0.7 论文分区实质取代容隙需求）。
  classic 侧只落地「union 模式选区当墙」（v0.7.23）：套索糊临时线再 tap 即可止漏；
  同批 fillgap toggle / 加线笔 一并挂起不做。
...
方案见 docs/20260528-lessons-magic-wand-gap-closing.md：Plan A(距离场+Dijkstra，窄缝高代价) > B(死端配对) > C(检测灾难性泄漏弹提示)。
...
> 出处：`docs/reports/20260609-store-cloud-sync-audit.md`（gitignored，细节带 file:line）。主干（If-Match/412-heal/push-serialize/move-aside）已验扎实；弱点全在 rename / 删除传播 / re-entry 边沿。
