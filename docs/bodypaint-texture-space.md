...
## 5.5 为什么不漂移（vs Procreate）—— WYSIWYG 的边界

**Procreate 的病**：在**掠射面**（quad 平面包含视线、边着看，屏幕上压成窄缝）和**非平面 quad** 上严重偏移。三个根因：
1. **掠射 = 映射雅可比爆炸 + 病态**。前缩（foreshortening）把一大片纹理压进几个屏幕像素 → 屏幕端亚像素误差/取整 → 纹理端大跳。且屏幕↔纹理映射在掠射处近奇异（det→0），求逆病态，数值误差被放大成肉眼偏移。
2. **非平面 quad 没有单一单应性**。homography 只对**平面**成立；4 角不共面的 quad 投影本质非线性。Procreate 若用 per-quad 线性/homography 近似反投影 → 中心系统性错位（**用线性映射近似本质非线性的映射**）。
3. **掠射处深度精度最差** → z-fighting → 可见性判定抖 → 漏/穿。

一句话：**Procreate 偏移 = 对一个病态/非单应映射做了 per-quad 线性近似 + 缓存，误差在掠射/非平面处被放大。**

**我们这套（texture-space gather）—— 分两层诚实回答：**

- **A. 位置对齐（无偏移）：保证。** 逐 texel 用**插值出的真实 world pos**（GPU 三角形透视正确插值）投影采样，不是 per-quad homography 近似 → 没有根因②那种系统偏移。**WYSIWYG 的结构保证 = Pass A（画）和 Pass B（烤）用同一份几何 + 同一相机矩阵**：画和烤是**同一个映射的两次精确求值**，不是"近似它"。两边投影逐字相同 → 你屏幕某点画的，必然烤进投到那点的那个 texel。逐 texel 精确、无漂移，掠射/非平面都不偏。
- **B. 分辨率/保真：掠射处不可能 WYSIWYG —— 但这是信息极限，不是 bug。** 掠射面屏幕只占几像素：一堆 texel 挤进同几个屏幕像素 → 采到同一个 stroke buffer 值 → 一笔被拉伸/糊成纹理一大片。**位置没错（就在你画的地方），保真低**。根本性的：没有足够屏幕像素承载细节，任何算法从该视角都救不了 → 解法 = 转正对再画 / 超采样。加根因③：掠射处 depth bias 最难调 → 可能 speckle，**仍不是偏移**。

**结论**：我们结构上**没有** Procreate 的漂移病（逐 texel 精确求同一映射）；掠射糊是物理极限，谁都有。**placement WYSIWYG 保证，掠射 fidelity 不保证（转视角解决）。**
