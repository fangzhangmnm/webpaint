...
v0.7.19 起 `inkDepth` **懒构建**（user：自动档吃灰，把内存省下来）：build 置 null，
首次 bleed≥0 查询时 oracle 重读像素 + 一遍 EDT 补算挂缓存（attachInkDepth）——
自动档（默认）零额外内存（2K 省 4MB）；regionMaskAt 在未 attach 时对 bleed≥0 响亮抛。
另：曾论证过「自动=排除中线脊」与「按半宽中位数自适应」两个统一默认方案，
user 2026-07-30 裁定**都不采**（脊沟在淡墨/软笔锋尖/后期调线稿层时露馅）——
自动=旧行为（填满到中线），像素画走底色图层或像素精确 flood，显式档留作 override。
