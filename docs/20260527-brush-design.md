...
User × Claude 讨论沉淀。和 [[20260526-brush-v0.md]] 不同：v0 写"现在的实现"，这份写"决策、为什么、还没做的"。
...
input.js 必须按 `ev.timeStamp` 严格单调过滤 raw event，否则 path arc-length 会被 Safari iOS 的 coalesced 边界回放注水 → 几十 doc-px 周期的疏密波。详见 [[20260527-brush-density-wave.md]] 和 [[20260527-ipad-coalesced-events.md]]。
