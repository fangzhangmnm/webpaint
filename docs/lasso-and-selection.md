...
- **transform 才是 transient**。Pending Transients 护栏管它（见 [pending-transients.md](pending-transients.md)）。
- 选区是 transform **的前一步**。
...
参考 [pending-transients.md](pending-transients.md) 区分 transient state 和持久 state。
