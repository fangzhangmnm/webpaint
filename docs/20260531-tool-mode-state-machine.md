...
> 领域词见 [CONTEXT.md](../CONTEXT.md) 的 Mode / Transient；undo 事务见 [20260527-undo-architecture.md](20260527-undo-architecture.md) + 已落地的 PixelEdit；
> transient 现状见 [20260528-pending-transients.md](20260528-pending-transients.md)（本设计吸收并简化它）。
...
[20260528-pending-transients.md](20260528-pending-transients.md) 设想"可同时多个 pending（lasso-floating + text-editing）"，用一个多项注册表 + `applyAllPendingTransients()`。
...
（见 [20260530-filter-plugin-architecture.md](20260530-filter-plugin-architecture.md)）全是**同一个 transient kind = `adjust`**，payload = 哪个 Filter + 参数 UI。
...
> 两者正交：① 别假设 transient = 一个 undo entry（盖印能产 N 个，见 [20260527-undo-architecture.md](20260527-undo-architecture.md)）；
...
20260528-pending-transients.md 早预警过这个 NM-if 问题。
...
- **undo 配额数**：用户要求查 memory（内存预算，见 [20260527-undo-architecture.md](20260527-undo-architecture.md) 内存预算节）再定，以后做。
