...
- `docs/20260527-artist-priorities.md:75/81`：形状工具（直线/椭圆/矩形）列为 anti-feature——「完全没人用」。
- v120（index.html 注释）：撤掉当时的 shapes 工具，判「shapes toggle 放在笔刷里面」。
- `docs/20260528-backlog.md`「智能形状笔」两条：Procreate 式实时手势识别 + 画完停手自动 snap。
...
- 缺 4 个图标（line / snap-angle / constrain-square / constrain-circle）走 icon-missing 占位，
  登记在 `docs/20260718-icon-todo.md`，进库后重跑 extract 即换真图。
