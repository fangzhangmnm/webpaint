...
**待续 → 接生产 board（需 iPad 在手验手感/视口/live 描边）**：
- board 渲染路径换 GLDocRenderer；board canvas 转 WebGL2；present 加**视口变换**(pan/zoom/rot，现 preview 是 1:1 fit)。
- live overlay 接 brush.getLiveOverlay（CPU canvas → 纹理每帧）；脏跟踪（只重传变更层，现 syncAll 全传）。
- **TileResidency**（压缩备份+OPFS autosave+context-loss 重上传；§4.1/4.2/4.4）— 配真 backend 才有意义。
- overlay 的 blendMode 变体（live 预览非常规笔刷）；组作 clip 基底（罕见）。
- **Stage 3**：GL 笔刷栅格化（StrokeRasterizer，消费 CPU StrokeSmoother 中心线）。
