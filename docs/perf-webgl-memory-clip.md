...
**Stage 2 待续 → 接 board（首个 iPad 可见，需真机验）**：
- doc.ts Layer(Canvas2D bbox)→tiles 桥 + 图层树→CompNode 翻译；board 渲染路径换 GLCompositor；board canvas 转 WebGL2。
- **TileResidency**（压缩备份+OPFS autosave+context-loss 重上传；§4.1/4.2/4.4）— 配真 backend 才有意义，此时做。
- overlay 的 blendMode 变体（live 预览非常规笔刷）。
