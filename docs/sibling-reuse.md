...
- `../../20260520 RealHome/` —— `docs/principles.md` 和 `docs/sync-constraints.md` 是跨项目宪法
...
| `apple-touch-icon` 必须 PNG | ScratchPad `docs/ios-pwa-quirks.md` | 用 ImageMagick 从 SVG 生成 180/192/512 PNG |
...
| PointerEvent 模式：palm rejection、coalesced events、平滑 α=0.65、屏幕双击切笔/橡皮、wheel 缩放、Space 临时 pan | ScratchPad `src/input.js` + `docs/pointer-and-pen-input.md` | 抄了行为矩阵和大部分 _down/_move/_up 骨架；不同的是画笔走 BrushEngine 而非矢量 stroke |
...
| sync 优先级宪法 | RealHome `docs/sync-constraints.md` |
