...
2. **导入图片到图层**：拖入文件 / 粘贴剪贴板 / 选文件，转成一层。photobash + ref。
3. **Clipping mask（真正的）**：每个图层可以 clip 到下面那层的 alpha。喷枪 / 渐变上色用。alpha lock 不够。
4. **Lasso + 自由变换**：套索 → move / scale / rotate / skew，进阶 perspective / mesh deform。最痛的痛点。
5. **液化**：推 / 拉 / 缩 / 旋 brush。比例 / 结构修复神器。
