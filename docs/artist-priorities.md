...
6. **长按直线 / 长按弧**（Procreate quick-assist）：笔画末尾停住 → 自动拉直 / 拉成圆弧。**不是形状工具**，是"拉直辅助"。
7. **图层模式**（multiply / overlay / screen / ...）：阴影 / 高光叠加。
8. **更多 brush preset**：硬笔 / 软喷枪 / 着色笔 / 厚涂笔，先 4-5 个够用。
9. **找颜色辅助 UI**：拉一个 "near current" 调色板 / 近色搜索（user 描述的"找颜色"动作）。
10. **剪贴板导入 / 导出**：
    - 导入：`navigator.clipboard.read()` 拿图 → 新建一层 + drawImage（同导入参考图）
    - 导出：`navigator.clipboard.write([new ClipboardItem({"image/png": blob})])`
    - iPad Safari / 桌面都支持。share popup 里加 "复制 PNG 到剪贴板" 一项。
11. **无缝贴图模式（offset / wrap）**：tileable texture 必备。
    - offset 视图：把 doc 居中点偏移 (W/2, H/2)，让 seam 在画面中央
    - wrap 笔刷：stamp 落在边缘时同时在反面镜像画一笔（modulo wrap），缝
      消失。这对 Blender / 3D 工作流是 niche 但非常 valuable —— tileable 是 PBR
      贴图基本要求。
    - 低优先级，等 cel-shading + lasso 稳了再做。

### Tier 3：兜底

10. **选区扩展操作**：羽化 / 收缩 / 膨胀 / 反选。
11. **图层蒙板**（除了 clip 还有 paint mask）。
12. **曲线 / Levels** 调整层。
13. **吸色拖动 + 放大镜**（user 提过但不急）。
