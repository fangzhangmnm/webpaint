...
> as-of v359 / 2026-06-29（§0 复核 + §3 已修；原 §1/§2 仍开）。**优化建议 + 起点**（会腐烂的 how 类，动手前先按 §0 量一遍）。地基已收成单一 GPU SSoT（见 docs/20260629-canvas-render-audit.md：全库无 CPU display 路径）——优化只针对 GPU 合成/warp 热路径，别去碰离屏 CPU（吸管/导出/缩略图/滤镜计算）。
