...
> **A/B 双路径（dev 面板 `firstOrder` live 切换对比，默认 A）**：
> - **A（默认，二阶 SmoothDamp）**：body 带 vel 动量；tail = vel 动量 flush = **弧**（真状态、不估算、帧间稳）。
> - **B（一阶线性）**：body 纯一阶 EMA（无 vel）；tail = out→pen **直线**。一阶不估算 heading 就只能线性
>   （v252 试过估算 heading 出弧的变体，转角手感不好已弃）。B 留着纯线性给手感对比用。
> 两者滞后都 ≈ 速度×tau、去抖/帧率无关一致；差别 = 转角 body 手感（A 带动量 / B 干净）+ tail（A 弧 / B 直）。
