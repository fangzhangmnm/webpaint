...
- **落地序**：① 抽 Substrate（**done**：共享原语下沉，行为保持，163 passed）→ ② `createWorkFileStore` + app cut-over（删 edits.*/setCloudDirty/transient/autosave-timers，改 store.edit/flush；**最大爆炸半径，同步手感真机验**）→ ③ `createFolderStore`（rack 切过去，status 取代 deriveRackCloudState）→ ④ L3 gallery-flow + 修 C1 → ⑤ L3 card view 浅 Vue。
