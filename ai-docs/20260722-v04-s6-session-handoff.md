> ⚠ 已过时留档（S7 已由第四棒完成，见 ai-docs/20260722-v04-s7-session-handoff.md）。原件写在 /tmp，2026-07-22 迁入 docs 防过夜蒸发。
...
1. **真机批（先于一切）**：三份清单同批交付——
   - `ai-docs/20260722-v04-batch1-handoff.md` §3（batch1 全家）
   - `ai-docs/20260722-v04-s5-selection-tiles.md` §3（选区 tile 化）
   - `ai-docs/20260722-v04-s6-float-workpiece.md` §4（浮层变换整链 + 三个行为变化）
...
2. `ai-docs/20260722-v04-batch1-handoff.md` — 路线图 + 现状地图 + 暗坑（§0 已更新到 S6 后状态）。
3. `ai-docs/20260722-v04-s6-float-workpiece.md` — S6 报告：**§2 三个行为变化是真机沟通重点**
   （transform 期 Ctrl+Z=history、reject=identity 写回 stamp 保留、lift 即清选区），§5 是 S7 交接点
   （floatSourceCanvas 懒物化 = bridge 化的替换点）。
4. `ai-docs/20260722-test-charter.md` — 历史 bug → 新架构保证的映射（S7 的 leaky-GPU 模拟测试维度在此）。
