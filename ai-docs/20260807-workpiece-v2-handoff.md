...
> **+ 该文 §9 的 B 骑士 2 条（第 13/14 条：`?nostore` 缺席模式 smoke + 正常模式回归）**
> + 下列 v2 新锚 10 条——**共 24 条**（§0 说的「14 条」= §7 的 12 + §9 的 2；2026-08-08 口径修正，
> 之前本节字面只点了 §7，照抄会漏 B 骑士那两条）。跑批时三份一起过；水印对 v0.8.23（或发版时的当前号）。
