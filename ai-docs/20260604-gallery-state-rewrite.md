...
> 架构 review：`ai-docs/reports/20260604-gallery-state-architecture-review.html`。
...
3. **不做拖拽**（iPad 触屏 drag-drop 不可靠，spec 也把触屏批选标 TBD）→ **卡片菜单加「移动到…」**：lockSyncGate 复用成 folder picker，移动 = 跨文件夹 rename（云端服务端 move 保 itemId/etag，无副本；身份 = path/name，GUID 方案 2026-06-07 已回滚，见 20260607-sync-identity-decision.md）。
...
5/6 是 sync 红线级，别只当 UI bug。参照 MyPWAPatterns/docs/20260604-potential-bugs.md + 20260528-sync-design.md。
