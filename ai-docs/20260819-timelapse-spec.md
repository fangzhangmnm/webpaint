...
> **裁决 2026-08-21（首个真视频 dogfood 后，user 原话：「实测结论是越后面单commit密度越高。所以我决定不用衰减，一直常数」）**：调和衰减**否决，平采样即终案**——固定时间窗对后期密 commit 天然多合并，方向已对。N₀ 常数与 `timelapseDebounceMs` 签名已从码里摘除（v0.10.12）；n 计数保留（纯统计，随 sidecar 持久化）。§2 衰减公式段落仅存档，永不生效。
> **dogfood 实测 2026-08-21（首个真视频「泉此方」，v0.10.11 录制，512 档 1:1）**：n=977 commits → motionSamples=488（比值 2.002，emergent 非结构——单监听单调用点已核实无双触发）；成片 489 帧 @10fps = 48.9s；mp4 953KB ≈ 0.91MiB，实测码率 156kbps（M 目标 250kbps，VBR 低走正常——本作平涂线稿偏简单）。对照 512 档选单口径「约 2MiB」：本样本偏 ~2×，在「画风差异 ~2×」承诺带内，样本 n=1 暂不动 refBytes，攒样本再校。
...
- **录制指示 = 安全护栏，全局常驻可见**（不许藏进菜单）：右下角状态栏，**只有**最小红点（CSS 呼吸）+ i18n 短词一条 key：`录` / `REC` / `録画` / `lukin`——**不显示体积**（user 2026-08-19：状态栏要最小）。点击直达录制控制项（暂停/体积实况/预览/清除都在那里）。
- **关 = 纯暂停**，已录素材一字节不动；**清除录像 = 独立显式按钮 + 应用内 inline 二次确认**（家规禁系统 confirm），清除**不可 undo**（非绘画操作，不进 undo 栈）。
- **暂停期间录像整体冻结**：继续画+保存**不刷新收尾帧**（刷=偷偷还在录）。尾帧停在暂停时刻，重开后新帧自然接上。
...
用户面对的全部技术面 = **比例 + 最长边 + 开关 + 清除**。事前选单参考体积、事中菜单录制控制项实况体积、事后嫌胖清除重录。

### 体积的写法（2026-08-19 grill 定稿）

- **选单参考数字 = 一位有效数字 + 「约」**（0.1/0.2/0.5/2/4/8 MiB）——精度姿态即诚实声明；档间比值是物理的（面积比），绝对值只承诺量级。
- **发版前硬前置：常数用 dogfood 真录像实测校准**（开发期宣发试画就是标定样本），把猜变成测；调和衰减已把画作长短的离散压到 <2×，剩余「约」只背画风差异 ~2×。
- **单位双层制（user 2026-08-19 拍板）**：
  - 数据/契约层 = **裸字节数**，物理良定义，防 abandonware 腐烂；代码常数一律字节，注释不许写裸 "MB"。
  - UX 显示层 = **1024 进位 + KiB/MiB 标签**（数值与 OneDrive 对账一致，标签不冒充 SI）。心理战规则：GB 以下单位永远 MiB（`0.3MiB` 不写 `300KiB`），不足 0.1MiB 显示 `<0.1MiB`，0.1–10 一位小数，10+ 整数。
- **支线登记（不在本 worktree，独立小 PR）**：`humanSize` 在 `src/gallery/gallery-shell.ts` 与 `src/gallery/gallery-view-model.ts` 有两份重复实现，且 1024 进位配 SI 标签（打架）——收敛成一个共享 util + 标签 KB→KiB。timelapse UI 从第一天用新约定，不继承病灶。
...
- 版本号：**user 2026-08-19 拍板落现 minor（0.9.x patch 递增）**，不触发 minor bump 流程（0.10 已插旗给混色）。
- **应用内预览进 v1**（user consent 2026-08-19）：`<video>` 标签喂 blob URL。防坑清单：`playsinline` 必加（iPad 全屏 PWA 防 iOS 原生全屏劫持）、blob URL 用完 `revokeObjectURL`（防泄漏）、controls 用原生（预览是工具窗不是产品面）、加载失败走 error-badge 不吞。
