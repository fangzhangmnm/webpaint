...
> **deploy race 注意**：如果几秒前刚 push 过 main，那次 main workflow 还在跑。`cancel-in-progress: true`（v130 加的）会自动取消它，让 prod push 触发的 workflow 重新跑、用最新两分支 tip 部署。如果手贱把 `cancel-in-progress` 改回了 false，main 那次会用旧 prod tree 抢先 deploy，prod 这次 success 也不生效——见 [20260530-lessons-pages-deploy-race.md](20260530-lessons-pages-deploy-race.md)。
...
- [ ] 抄 `.github/workflows/deploy.yml`（**确认 `concurrency.cancel-in-progress: true`**，false 会踩 deploy race，[20260530-lessons-pages-deploy-race.md](20260530-lessons-pages-deploy-race.md)）
...
3. **Actions concurrency race（v130 踩到的）**：两个分支几乎同时 push（典型场景：promote prod 时先 push main 再 merge push prod，间隔 10 秒）会触发 **deploy race**：
   ...
   - 详细复盘 [20260530-lessons-pages-deploy-race.md](20260530-lessons-pages-deploy-race.md)
