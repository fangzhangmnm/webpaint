...
2. ➜ 抽库/分发机制已科普并列谱系：见 `20260802-v08-lib-distribution-grill.md`（流派未选定；对接 recon-e 拍板项 5）。
3. 顺手账（无争议，待排期）：brush.ts 压感 LPF 壁钟 dt → 事件 t（回放前提）。
4. 未拷问：B 的 password 设计（政策在 app/机制在 store 的分界动不动）、mhtml 可行性细节、E 的列举接口统一（watchFolder vs listAllItems）、术语拆分张力（见上）、F 的 i18n 插件自带文案问题。
