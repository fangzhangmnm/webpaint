...
把 fx 的注册表接缝（filters.js）泛化时，曾提议把 crop / flip / resize（**Op**）和 brush/eraser/picker/lasso（**Tool**）也收成插件注册表（见 ai-docs/reports/20260608-ui-deepening-and-plugin-survey.html 候选 2）。**两者都决定不做**——经论证，它们不是缺失的抽象，已被既有深接缝覆盖；再建框架是浅包装。**真正做成 contribution 插件的只有 Exporter（已落地）；Filter 本就是。**
