...
- **决策 + 实现 gap 清单**：`20260813 internal-store/ai-docs/adr/0016-clean-silent-fast-forward-conflict-on-dirty.md`（status: accepted / **NEEDS REFACTOR**）。它的 "NEEDS REFACTOR" 段就是 touch-point 清单。**先读它。**
...
- **真机**（用户跑，你跑不了浏览器）：串行两设备交接不刷 backup；干净端切回来内容自动变最新；并发双屏才弹冲突。报告写 `ai-docs/reports/`（gitignored）。
