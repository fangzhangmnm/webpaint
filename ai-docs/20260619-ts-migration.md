...
> as-of v319 / 2026-06-20。本文是 how 类文档（最易腐烂）——与代码矛盾时信代码（`tsconfig.json` 的 `include` 是唯一真相）。
> **v319：app.ts 的 ctx 组合根去 cast——`as unknown as AppContext` → `const ctx: AppContext`（真验证）。** 收掉 AppContext 与现实的全部漂移：① gallery/leftDial 弃本地镜像，`import type` ui/gallery·ui/left-dial 真句柄（单一真源）；② rack/referenceWindow/paletteWindow 去 `[k:string]` index sig（真 class 无 index sig 故装不进，去掉后直接 assignable）；③ `_suppressTransientPanels` 改 `(mode: string)` 必填；④ 12 个 `(...args:unknown[])` 占位换真签名（updateNewerBanner/layerSpecFrom/_bringPanelTop/selectionToNewLayer/importImageAsLayer/afterDocChange/applyCheckerboard/gateCloudSyncOnOpen/checkQuotaAndWarn/uniqueLocalName/getLocalSavedAtLabel/showFullscreenBusy）。级联 5 个消费方 call-site type-only cast 收口（layers-panel ×3 / selection-ops / session-state）。**组合根从此 tsc 验证而非盲信——加/漏 ctx 键、签名错配编译器即点出。** tsc 0 + 388 测试 + bundle。（仅 line-105 `rackStore as unknown as` 构造接缝留存，与 ctx 无关。）
> **v318：红线 `store/**` 的 12 个 any 收紧（用户 escalate 批准）。** `catch(e:any)`→`unknown`+点用 cast；`(blob as any).size/.length/.slice`→`local.get` 真返回 Blob + 防御 cast；`(cloud as any).pullTail`→真 `CloudSync.pullTail`；`(local!.save as any)`→去掉（sealed:Bytes 本就 match）；`(crypto as any).randomUUID`→lib.dom 真有；`hint?:any`→`unknown`（match interface）；cloud-sync 字节读 union cast。**铁律：仅注解，4 个红线文件全部 transpile-diff 验证 emitted-JS 逐字不变（byte-identical），200+ store 测试全绿。** 全 src `any` 17→5（剩 app-store rackDecode JSON.parse 2 / zip vendor 1 / brushes IDB 迁移 1 等诚实边界）。
> **🎉🎉 v316（batch 17，全收官）：`src/plugins/*.js`（7 个第一方滤镜插件 hsb/color-balance/curves/sharpen-blur/liquify/stylize_filters/index）→ `.ts`，全部 0 any，按 `filters.ts` 的 `Filter` 契约类型化（class static 侧 satisfies Filter；liquify 无 bake 走 `as unknown as Filter` 注册）。`app.ts` 的 `import "./plugins/index.ts"` 同步。至此 `src/**` 100% TypeScript、全部 gated、0 个 @ts-suppress。JS→TS 迁移彻底完成。** 残留 any 仍是那 17 个诚实边界（12 红线 store + app-store/zip/brushes），plugins 贡献 0。
> **v315：`version.js`→`version.ts`（deploy 版本 sed 目标随之改为 `src/version.ts`）+ gate 补 4 个 ui 叶（brush-settings/-model/brush-size/left-dial，原 checked-via-import 现显式入门）。至此 `src/**` 全 `.ts`（零 .js）。** 剩余 `any` 17 个全特征化：12 在红线 store/**（store.ts 9 等，error/duck-typing/optional-probe 边界，改前 escalate）、2 app-store rackDecode JSON.parse、1 zip vendor、1 brushes IDB 迁移——皆诚实边界。0 个 @ts-ignore/nocheck。`src/plugins/*.js`（7 个第一方滤镜插件）仍 .js = app.ts `import "./plugins/index.js"` side-effect 引入的未 type-check 岛，下一批可选。
> **🎉 v313（batch 16，收官）：`app.js`（组合根）+ `app-store.js`（红线门面）入门 → JS→TS 迁移完成。** src 下仅剩 `version.js`（故意保 .js：deploy/build 按路径 sed 它）。app.ts 把引擎单例 + ctx 字面量类型化（ctx `as unknown as AppContext`——`app-context.ts` 的 `GalleryHandle.setView`/`CurrentBrushRef` 与真实现有漂移，留作后续 reconcile 让 ctx 能直验）。app-store.ts 红线：仅加注解，subagent 用 transpile-diff 证明 emitted-JS 逐字不变；2 个 any 在 `rackDecode` JSON.parse 边界。build.sh ENTRY → `./src/app.ts`。tsc 0 + 388 测试 + bundle。
...
> 完整勘探报告：`ai-docs/reports/2026-06-19-js-ts-migration-deepening-review.html`（gitignored，仅本机）。
