...
- **配套云侧坑（同族，autosave 完整性）**：
  - R4 revert 绕过 clean→dirty 门 → 快进无备份吃掉 revert 结果。出处：`ai-docs/reports/20260610-cloud-data-safety-audit-v228.md:35-39`。
...
- **旧架构根因（三重）**：
  1. **operator 无逆元强制**：α 命令模式靠各模块自觉注册 handler，图层组 op 同时碰多个图层，
     ...
     `ai-docs/20260527-undo-architecture.md`（handler 注册分散、纪律靠人守）；当前实现
...
- **旧架构根因**：clear 的历史语义是"不可撤销，烧掉"——旧策略文档明说 clear button =
  `clearActiveLayer + clearHistory`，**不进 undo**。出处：`ai-docs/20260526-undo-strategy.md:56`。
...
- **旧架构根因**：图层 id 是**数字单调计数器且会复用**。当前 `src/doc.ts:18`
  ...
  **主动让删掉的层"用同一个 id 复活"**（`ai-docs/20260527-undo-architecture.md:139` layerSpec
...
- **现象**：非崩溃 bug，是长期内存压力。旧 snapshot 链 20 步 = 320MB。出处：`ai-docs/20260526-undo-strategy.md:7`、`:24-33`。
...
- **`undo 复活的层拿新 id，旧 entry 不误伤`**
  ...
  - 断言：复活层 id ≠ idA（**否决旧 `ai-docs/20260527-undo-architecture.md:151` 的"同 id 复活"**）；
