...
> 纪律提醒（沿用本仓）：结论对回**源码行号**，别信任另一个 AI（含上一个我）的总结；lasso 是历轮幻觉重灾区（见下 K4）。`/improve-codebase-architecture` 报告按用户要求落 `ai-docs/reports/`（gitignored），**不开浏览器**。
...
- 领域词表：`CONTEXT.md`。同步/存储模型的账本：`../20260601 MyPWAPatterns/docs/MASTER.md` + 家族 ADR 集 0001-0017（家族级在 MyPWAPatterns/docs/adr/，store 系已迁 20260813 internal-store/ai-docs/adr/，2026-08-13 分家）（**只是 ADR=决策账本；WebPaint 的 `src/store` 才是 SSoT 实现**）。
...
## 5. Suggested skills
- **`/improve-codebase-architecture`**（主任务）：start fresh 钻探，但**别再 propose**：K1（引擎 registry 已做）、K4（lasso mask 代数=幻觉，已在 selection.js）、已下沉的 doc/crop/gallery/rack 模块。可深挖：store→repo facade 落地、L3 gallery-flow、剩余 god-file cluster。报告落 `ai-docs/reports/`，不开浏览器。
