...
export function installDomShim() {
  ...
  const win = {
    ...
    // 真事件发射器（测试需 dispatch window 事件，如 wp:adjsize）。boot 期 app 只注册不 dispatch，无害。
    _listeners: new Map(),
    addEventListener(type, fn) { if (!win._listeners.has(type)) win._listeners.set(type, new Set()); win._listeners.get(type).add(fn); },
    removeEventListener(type, fn) { win._listeners.get(type)?.delete(fn); },
    dispatchEvent(ev) { win._listeners.get(ev?.type)?.forEach((fn) => fn(ev)); return true; },
    scrollTo: NOOP,
