...
describe("cloud-sync.push H7 兜底 · 对抗（不得把 0 字节占位当成功）", () => {
  ...
  it("真 H7（分片末无 item 但字节确实上传到位）→ 大小匹配 + 尾相符 → 认，标 synced", async () => {
    const fake = {
      upload: async () => null,                                                 // 末响应没带 item
      getItemByPath: async () => ({ id: "x", name: "猫.ora", eTag: "e2", size: 5 }),  // 但云端确有 5 字节
      // N6：认领前尾部校验需拉云端尾字节——云端确实存着我方上传的 "12345"（尾相符 → 确认我方 push）。
      downloadRange: async (_id, offset, len) => new TextEncoder().encode("12345").slice(offset, offset + len),
    };
    const cs = createCloudSync({ provider: fake, kv: memKv(), fileName: (n) => n + ".ora", appKey: "t" });
    cs.setDirty("猫", true);
...
describe("cloud-sync.listAll complete 标志（cloud-gone reconciliation 的 partial-list 守卫）", () => {
  const mkItem = (x, p) => ({ ...x, path: p ? `${p}/${x.name}` : x.name, size: 1, eTag: "e", lastModifiedDateTime: 0 });
  const mkCloud = (tree) => createCloudSync({
    provider: { list: async (p) => { const v = tree[p ?? ""] ?? []; if (v instanceof Error) throw v; return v.map((x) => mkItem(x, p)); } },
    kv: memKv(), fileName: (n) => n + ".ora", match: (it) => /\.ora$/i.test(it.name || ""), toName: (p) => p.replace(/\.ora$/i, ""),
  });

  it("全成功 → complete=true", async () => {
    const cloud = mkCloud({ "": [{ id: "a", name: "a.ora", isFolder: false }] });
    const r = await cloud.listAll();
    eq(r.complete, true); eq(r.files.length, 1);
  });

  it("子文件夹列举失败 → complete=false（partial：缺失≠云端真没了，reconcile 据此不收敛）", async () => {
    const cloud = mkCloud({
      "": [{ id: "f", name: "sub", isFolder: true }, { id: "a", name: "a.ora", isFolder: false }],
      "sub": new Error("mock 500"),          // 子树列举失败 → 吞掉 → partial
    });
    const r = await cloud.listAll();
    eq(r.complete, false, "partial 列表必须 complete=false");
  });

  it("根列举失败 → complete=false", async () => {
    const cloud = mkCloud({ "": new Error("mock 500") });
    const r = await cloud.listAll();
    eq(r.complete, false);
  });
});
