...
describe("workpiece-layer-tree · 装配纪律", () => {
  it("未装配访问 workpiece.layers → throw；重复装配 → throw", () => {
    const { w, h, wp2, doc } = (() => {
      let unrec = 0;
      const h = new LegacyHistory({ maxQuotaBytes: 1 << 20, onUnrecoverable: () => { unrec++; } });
    ...
    new LayerTree({ w, history: h, tree: wp2.layerTree, tiles: wp2.layerTiles, port: doc });
    void w.layers;   // 装配后可访问
    let threw2 = false;
    try { new LayerTree({ w, history: h, tree: wp2.layerTree, tiles: wp2.layerTiles, port: doc }); } catch { threw2 = true; }
