...
# (id, 文字, 盒, 语义 note)
SPECS = [
    # line/snap-angle/constrain-* 已进库（2026-07-25）。「perspective 透」已撤（UI v2 改成模式行）。
    # 透视模式行（transform 风格四图标）：
    ("persp-viewport", "视", (3, 3, 18, 18), "形状笔·透视模式:视口对齐(关透视)。真图标建议=视口框/无汇聚"),
    ("persp-1p", "一", (3, 3, 18, 18), "形状笔·透视模式:一点透视。真图标建议=单消失点汇聚线"),
    ("persp-2p", "二", (3, 3, 18, 18), "形状笔·透视模式:二点透视。真图标建议=双消失点"),
    ("persp-3p", "三", (3, 3, 18, 18), "形状笔·透视模式:三点透视。真图标建议=三消失点"),
    # 平面行：
    ("plane-ground", "地", (3, 3, 18, 18), "形状笔·透视平面:地板(透视 grid)。真图标建议=透视地面格"),
    ("plane-wall", "墙", (3, 3, 18, 18), "形状笔·透视平面:纵深墙(一点透视)。真图标建议=纵深墙面"),
    ("plane-wall-left", "左", (3, 3, 18, 18), "形状笔·透视平面:左墙。真图标建议=左侧墙面"),
    ("plane-wall-right", "右", (3, 3, 18, 18), "形状笔·透视平面:右墙。真图标建议=右侧墙面"),
    # VP 编辑入口：
    ("vp-edit", "VP", (2.5, 4.5, 19, 15, "sans"), "形状笔·编辑消失点按钮(进 crop 同款半模态)。真图标建议=点+十字/拖拽把手"),
    ("reset-defaults", "重", (3, 3, 18, 18), "VP 编辑·重置默认(VP 回默认位+锁回开+box 回默认)。真图标建议=回环箭头+点"),
    ("snap-vp", "吸", (3, 3, 18, 18), "形状笔·直线约束在透视下的换义图标(吸向消失点)。真图标建议=线段收向小圆点"),
]
syms = []
